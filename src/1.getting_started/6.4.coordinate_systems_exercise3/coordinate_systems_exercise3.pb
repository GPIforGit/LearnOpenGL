...
Procedure main()
  ...
  *surface = sdl::IMG_Load("../../../resources/textures/awesomeface.png")
  ...
  While Not window::ShouldClose()
    ...
    Protected.math::mat4x4 view, projection
    math::Mat4x4_set_Scalar(view, 1.0) ; make sure to initialize matrix to identity matrix first
    math::Mat4x4_set_Scalar(projection, 1.0)
    
    math::perspective(projection, Radian(45), window::GetAspect(), 0.1, 100.0)
    math::translate_float(view, view, 0.0, 0.0, -3.0)
    ; pass transformation matrices to the shader
    gl::UniformMatrix4fv(projectionLoc, 1, #False, projection); note: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
    ...
    For i=0 To sizeCubePosition-1
      ...
      If (i % 3) = 0 ; every 3rd iteration (including the first) we set the angle using GLFW's time function.
        angle + sdl::ext_GetElapsedSeconds() * 25.0
      EndIf
