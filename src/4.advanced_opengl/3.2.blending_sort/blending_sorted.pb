...
Procedure main()
  ...
  ; -------------------------
  Protected.l Shader = shader::new("blending.vs", "blending.fs")
  ...
  Protected.l transparentTexture = texture::load("../../../resources/textures/window.png",gl::#CLAMP_TO_EDGE,gl::#CLAMP_TO_EDGE);
  ...
  DataSection
    ...
    WindowEnd:
  EndDataSection
  Protected.math::vec3Array *window = ?window
  Protected.l sizeWindow = (?windowEnd-?window) / SizeOf(math::Vec3)
  ...
  Structure sSorted
    *window.math::Vec3
    distance.f
  EndStructure
  Dim sorted.sSorted(sizeWindow-1)
  ...
  While Not window::ShouldClose()
    ...
    ; ---------------------------------------------
    Protected.l i
    Protected.math::vec3 *CameraPos = camera::GetPosition(*camera)
    For i=0 To sizeWindow-1
      sorted(i)\distance = math::vec3_distance2( *CameraPos, *window\v[i])
      sorted(i)\window = *window\v[i]
    Next
    SortStructuredArray( sorted(), #PB_Sort_Descending, OffsetOf(sSorted\distance), #PB_Float)
    ...
    *view = camera::GetViewMatrix(*camera)
    ...
    For i=0 To sizeWindow-1
      math::mat4x4_set_Scalar(model,1)
      math::translate(model, model, sorted(i)\window )
      shader::setMat4x4(shader,"model", model)
      gl::DrawArrays(GL::#TRIANGLES, 0, 6)
    Next
