...
;  - only rotate yaw, not yaw and pitch
;  - inverted the horizontal texture-coordinate of the quadVertices - to mirror things
;  - Aspect ration is handled correct and add viewport
...
Procedure main()
  ...
  While Not window::ShouldClose()
    ...
    Protected.f yaw, pitch
    camera::GetAngle(*camera, @yaw, @Pitch) ;rotate the camera's yaw & pitch 180 degrees around
    camera::SetAngle(*camera, yaw+180, pitch)        
    math::mat4x4_set(view, camera::GetViewMatrix(*camera))    
    camera::SetAngle(*camera, yaw, pitch) ; reset original 
    
    math::perspective(projection, Radian(camera::GetZoom(*camera)), window::GetAspect() * (0.6/0.3), 0.1, 100.0); multiply the aspect of the mirror!
